...
switch ($_GET["op"]) {
    ...
    case "insert":
        $datos = $ticket->insert_ticket($_POST["usu_id"], $_POST["req_id"], $_POST["cat_id"], $_POST["opci_id"], $_POST["tick_titulo"], $_POST["tick_descrip"]);
    ...
    case "listar_x_usu":
        ...
        foreach ($datos as $row) {
            ...
            //condicion para saber si el ticket es solicitado y tambien terminado
            if ($row["tick_estado"] == "Solicitado") {
                $sub_array[] = '<span class="label label-pill label-success">Solicitado</span>';
            } else {
                $sub_array[] = '<a onClick="CambiarEstado(' . $row["tick_id"] . ')"><span class="label label-pill label-danger">Terminado</span></a>';
            }

            //fecha de creacion del ticket
            $sub_array[] = date("d/m/Y H:i", strtotime($row["fech_crea"]));

            //condicion para asignar fecha al ticket
            ...
            //condicion para cerrar el ticket
            ...
            //condicion para asignar el ususario
            ...
            //condicion para asignar usuario y a la vez realizar la tarea
            if ($row["usu_asig"] == null) {
                $sub_array[] = '<span class="label label-pill label-warning">Sin Asignar</span>';
            } else {
                $datos1 = $usuario->get_usuario_x_id($row["usu_asig"]);
    ...
    case "listar":
        ...
        foreach ($datos as $row) {
            $sub_array = array();
            $sub_array[] = $row["tick_id"]; // el id del ticket = tm_ticket
            $sub_array[] = $row["tick_titulo"]; //el titulo del requerimiento = tm_ticktitulo
            $sub_array[] = $row["req_nom"];
            $sub_array[] = $row["cat_nom"]; //el nombre de la categoria = tm_categoria
    ...
    case "mostrar";
        ...
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
                ...
                if ($row["tick_estado"] == "Solicitado") {
                    $output["tick_estado"] = '<span class="label label-pill label-success">Solicitado</span>';
                } else {
                    $output["tick_estado"] = '<span class="label label-pill label-danger">Terminado</span>';
                }
                ...
                $output["sop_nom"] = $row["sop_nom"];
                $output["sop_ape"] = $row["sop_ape"];
