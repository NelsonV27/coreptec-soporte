...
switch ($_GET["op"]) {
    ...
    case "guardaryeditar":
        if (empty($_POST["cat_id"])) {
            $categoria->insert_categoria($_POST["req_id"], $_POST["cat_nom"]);
        } else {
            $categoria->update_categoria($_POST["cat_id"], $_POST["req_id"], $_POST["cat_nom"]);
        }
        break;

    case "listar":
        $datos = $categoria->get_categoria_all();
        $data = array();
    ...
    case "combo":
        $datos = $categoria->get_categoria_x_tick_titulo($_POST["req_id"]);
        $html = "";
        $html .= "<option label='Seleccionar'></option>";
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
