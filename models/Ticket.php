<?php
    class Ticket extends Conectar{
        ...
        public function insert_ticket($usu_id,$req_id,$cat_id,$opci_id,$tick_titulo,$tick_descrip){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="INSERT INTO tm_ticket
            (tick_id, usu_id, req_id, cat_id, opci_id, tick_titulo, tick_descrip, tick_estado, fech_crea, usu_asig, fech_asig, tick_estre, tick_coment, fech_cierre, est)
            VALUES(NULL,?,?,?,?,?,?,'solicitado',now(),NULL,NULL,NULL,NULL,NULL,'1');";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $usu_id);
            $sql->bindValue(2, $req_id);
            $sql->bindValue(3, $cat_id);
            $sql->bindValue(4, $opci_id);
        ...
        public function listar_ticket_x_id($tick_id){
            ...
            tm_usuario.usu_nom,
            tm_usuario.usu_ape,
            tm_soporte.usu_nom as sop_nom,
            tm_soporte.usu_ape as sop_ape,
            tm_categoria.cat_nom FROM tm_ticket
            INNER JOIN tm_categoria ON tm_ticket.cat_id = tm_categoria.cat_id
            INNER JOIN tm_usuario AS tm_usuario ON tm_ticket.usu_id = tm_usuario.usu_id
            LEFT JOIN tm_usuario AS tm_soporte ON tm_ticket.usu_asig = tm_soporte.usu_id
            INNER JOIN tm_requerimiento ON tm_ticket.req_id = tm_requerimiento.req_id
            WHERE tm_ticket.est = 1 AND tm_ticket.tick_id = ?";
            $sql=$conectar->prepare($sql);
        ...
        public function insert_ticketdetalle_reabrir($tick_id,$usu_id){
            ...
            parent::set_names();
                $sql="	INSERT INTO td_ticketdetalle 
                    ...
                    VALUES 
                    (NULL,?,?,'Ticket Re-Solicitado...',now(),'1');";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $tick_id);
            $sql->bindValue(2, $usu_id);
            $sql->execute();
        ...
        public function get_ticket_totalabierto(){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT COUNT(*) as TOTAL FROM tm_ticket where tick_estado='Solicitado'";
            $sql=$conectar->prepare($sql);
            $sql->execute();
            return $resultado=$sql->fetchAll();
        }
        ...
        public function get_ticket_totalcerrado(){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT COUNT(*) as TOTAL FROM tm_ticket where tick_estado='Terminado'";
            $sql=$conectar->prepare($sql);
            $sql->execute();
            return $resultado=$sql->fetchAll();
        } 
        ...
        public function filtrar_ticket($req_id,$cat_id){
            $conectar= parent::conexion();
            parent::set_names();
